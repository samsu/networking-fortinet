...
def rollback_on_err(obj, context, err):
...
def allocate_vdom(obj, context, **kwargs):
    return add_record(obj, context, fortinet_db.Fortinet_ML2_Namespace,
                      **kwargs)
...
def add_routerstatic(obj, context, **kwargs):
    record = allocate_routerstatic(obj, context, **kwargs)
    kwargs.pop('subnet_id', None)
    res = add_resource_with_id(obj, context, record,
                               resources.RouterStatic, **kwargs)
    if not getattr(record, 'edit_id'):
        record.update_record(context, record, edit_id=res['results']['mkey'])
    return record


def allocate_routerstatic(obj, context, **kwargs):
...
def allocate_vlink_intf(obj, context, vlink_vlan, vlink_ip):
    '''
    :param obj:
    :param context:
    :param vlink_vlan:
...
def allocate_vlink(obj, context, fortigate_id, vdom):
    """
    :param obj:
    :param context:
    :param fortigate_id:
    :param vdom:
    :return:
        query and allocate resources from remote fortigates to use.
    """
    vlink_vlan = add_record(obj, context,
                        fortinet_db.Fortinet_Vlink_Vlan_Allocation,
                        vdom=vdom)
    ...
    vlink = {}
    if vlink_ip:
        vlink = allocate_vlink_intf(obj, context, vlink_vlan, vlink_ip)
        gateway_ip = get_ipaddr(netaddr.IPNetwork(vlink_ip.vlink_ip_subnet), 1)
        route = allocate_routerstatic(obj, context,
                                      fortigate_id=fortigate_id,
                                      vdom=vdom,
                                      dst=const.EXT_DEF_DST,
                                      device=vlink_vlan.inf_name_int_vdom,
                                      gateway=gateway_ip)
        vlink['routestatic'] = route.make_dict() if route else {}
    return vlink
