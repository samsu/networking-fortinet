...
class NetworkScenarioTest(ScenarioTest):
    ...
    def _get_server_port_id_and_ip4(self, server, ip_addr=None):
        ...
        if inactive:
            LOG.warning(_LW("Instance has ports that are not ACTIVE: %s"),
                        inactive)
    ...
    def check_floating_ip_status(self, floating_ip, status):
        ...
        self.assertEqual(status, floating_ip.status,
                         ...
                         .format(fp=floating_ip, cst=floating_ip.status,
                                 st=status))
        LOG.info(_LI("FloatingIP: {fp} is at status: {st}")
    ...
    def _check_remote_connectivity(self, source, dest, should_succeed=True,
        ...
        def ping_remote():
            try:
                ...
                LOG.warning(_LW('Failed to ping IP: %(dest)s via a ssh '
                            'connection from: %(host)s.'), {'dest': dest,
                            'host': source.ssh_client.host})
