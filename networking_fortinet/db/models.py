...
class DBbase(object):
    ...
    def add_record(cls, context, **kwargs):
        ...
        if not context:
            record = cls._record_with_kwargs(**kwargs)
            return {'result': record, 'rollback': {}}
    ...
    @staticmethod
    def _record_with_kwargs(cls, **kwargs):
        record = cls()
        for key, value in six.iteritems(kwargs):
            setattr(record, key, value)
        return record
    ...
    def delete_record(cls, context, kwargs):
        ...
        **kwargs because of the requirements of rollback process.
        ...
        """
        if not context:
            return cls._record_with_kwargs(**kwargs)
    ...
    def make_dict(self):
        ...
        for name in dir(self.__class__):
            ...
            if isinstance(value, orm.attributes.InstrumentedAttribute):
                dict[name] = getattr(self, name, None)
        return dict


class Fortinet_Fortigate(model_base.BASEV2, models_v2.HasId, DBbase):
    """Schema for Fortigates."""
    # id is the fortigate id, it is allocated by neutron server and associated
    # with l3_agent_id(compute node)
    host = sa.Column(sa.String(255), default=None)
    #agent_id = sa.Column(sa.String(36),
    #                     sa.ForeignKey('agents.id', ondelete="CASCADE"),
    #                     nullable=True)
    fortigate_ip = sa.Column(sa.String(32), default=None)
    status = sa.Column(sa.String(11), default="enable")


class Fortinet_ML2_Namespace(model_base.BASEV2, DBbase):
    """Schema for Fortinet network."""
    id = sa.Column(sa.Integer, primary_key=True, autoincrement=True)
    #fortinet_id = sa.Column(sa.String(36),
...
class Fortinet_Vlink_Vlan_Allocation(model_base.BASEV2, models_v2.HasId,
    ...
    def add_record(cls, context, **kwargs):
        ...
        if not record:
            ...
            # TODO(samsu): if no vlanid needed, then it should be able to add
