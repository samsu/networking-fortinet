...
class FortinetOVSBridge(ovs_lib.OVSBridge):
        ...
        if isinstance(add_attr, dict):
            if isinstance(cur_attr, dict):
                for field, value in add_attr.iteritems():
                    ...
                    cur_attr[field] = value if not cur_value else \
    ...
    def get_namespaces(self, port_name=None, column=None,
                       check_error=True, log_errors=True):
    ...
    def get_namespace(self, port_name=None, port_id=None,
        ...
        subnet_id = self._get_subattr(cur_attr, attr_path, formatted=True)
    ...
    def get_subattr(self, table, port_name, column, attr_path,
                       check_error=True, log_errors=True):
        """
        :param table:
        :param port_name:
        ...
        cur_attr = self.db_get_val(table, port_name, column,
                                   check_error=check_error,
                                   log_errors=log_errors)
        return self._get_subattr(cur_attr, attr_path, formatted=False)
